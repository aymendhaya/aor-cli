...
if (cl.valid && cl.operation === "create") {
  if (!fs.existsSync(cl.module_name)) {
    ...
    fs.appendFile(cl.module_name + "/index.js", aorIndex(cl.add), function(
      err
    ) {
      if (err) throw err;
    ...
    cl.add.map((filename, index) => {
      ...
      if (index === cl.add.length - 1)
        setTimeout(function() {
          let listProp =
            cl.add.indexOf("List") > -1 ? `list="${cl.module_name}.List"` : "";
          let editProp =
            cl.add.indexOf("Edit") > -1 ? `edit="${cl.module_name}.Edit"` : "";
          let createProp =
            cl.add.indexOf("Create") > -1
              ? `create="${cl.module_name}.Create"`
              : "";
          let showProp =
            cl.add.indexOf("Show") > -1 ? `show="${cl.module_name}.Show"` : "";
          console.log(
            chalk.yellow(`
...
function aorIndex(subModules) {
  let _imports = subModules
    .map(subModule => `import ${subModule} from './${subModule}';`)
    .join("\n");
